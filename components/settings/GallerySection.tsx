...
export default function GallerySection({ formData, handleUpdate }: any) {
  ...
  const handleSave = () => {
    if (!formData.picBedUrl || !formData.picBedToken) {
      showToast("API 地址和 TOKEN 不能为空", "error");
      return;
    }
    // 统一使用顶部保存按钮
  };
  ...
  return (
    <motion.section initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -10 }} className="w-full bg-white/40 dark:bg-slate-900/40 backdrop-blur-2xl border border-white/50 dark:border-slate-800/50 rounded-[40px] p-8 shadow-2xl">
