 ...
 * @description 管理网站配置，带 CloudBase 错误提示
 */
...
export default function SettingsPage() {
  ...
  useEffect(() => {
    if (!cloudBaseReady || !db) {
      setErrorMsg('云端服务未就绪：' + (cloudBaseError || '连接中'))
      setIsLoading(false)
      return
    }
    db.collection('siteConfig')
      ...
      .catch((err: any) => {
        ...
        setErrorMsg('加载配置失败: ' + err.message)
      })
  ...
  const handleSave = async () => {
    if (!cloudBaseReady || !db) {
      setToast({ message: '❌ 云端服务未就绪：' + (cloudBaseError || '连接中'), type: 'error' })
      return
    }
    setIsSaving(true)
    try {
      ...
      if (existing.data && existing.data.length > 0) {
        await db.collection('siteConfig').doc((existing.data[0] as any)._id).update({ ...formData, updatedAt: new Date() })
      } else {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <header className="bg-white shadow">
        <div className="max-w-5xl mx-auto px-4 py-4 flex justify-between items-center">
          <div className="flex items-center gap-4">
            <button onClick={() => navigate('/')} className="text-gray-500 hover:text-gray-700 text-sm cursor-pointer">← 返回</button>
            <h1 className="text-xl font-bold text-gray-900">⚙️ 站点配置</h1>
          </div>
          <button onClick={handleSave} disabled={isSaving} className="px-4 py-2 border border-transparent text-sm font-medium rounded-md text-white bg-blue-600 hover:bg-blue-700 disabled:opacity-50 cursor-pointer">
            {isSaving ? '保存中...' : '💾 保存到云端'}
          </button>
        </div>
      </header>
      ...
      <main className="max-w-5xl mx-auto px-4 py-8">
        {errorMsg && (
          <div className="mb-4 bg-yellow-50 border border-yellow-200 text-yellow-800 px-4 py-3 rounded text-sm">⚠️ {errorMsg}</div>
        )}
            ...
            <div className="bg-white rounded-md shadow p-3 flex md:flex-col gap-2">
              {tabs.map((tab) => (
                <button key={tab.id} onClick={() => setActiveTab(tab.id)}
                  className={`flex items-center gap-2 px-4 py-2.5 rounded-md text-sm font-medium transition-all w-full text-left cursor-pointer ${
            ...
            {activeTab === 'profile' && (
              <div className="bg-white rounded-md shadow p-6 space-y-6">
                ...
                <div>
                  ...
                  <input type="text" value={formData.author} onChange={(e) => handleUpdate('author', e.target.value)} placeholder="你的名字"
                    className="w-full px-3 py-2 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">个人简介</label>
                  <textarea value={formData.bio} onChange={(e) => handleUpdate('bio', e.target.value)} placeholder="一句话介绍自己" rows={3}
                    className="w-full px-3 py-2 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" style={{ resize: 'none' }} />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">头像链接</label>
                  <input type="url" value={formData.avatarUrl} onChange={(e) => handleUpdate('avatarUrl', e.target.value)} placeholder="https://example.com/avatar.jpg"
                    className="w-full px-3 py-2 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
                  {formData.avatarUrl && (
            ...
            {activeTab === 'site' && (
              <div className="bg-white rounded-md shadow p-6 space-y-6">
                ...
                <div>
                  ...
                  <input type="text" value={formData.title} onChange={(e) => handleUpdate('title', e.target.value)} placeholder="你的博客标题"
                    className="w-full px-3 py-2 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">网站描述</label>
                  <textarea value={formData.description} onChange={(e) => handleUpdate('description', e.target.value)} placeholder="一句话描述你的博客" rows={3}
                    className="w-full px-3 py-2 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" style={{ resize: 'none' }} />
                </div>
              </div>
            )}
              ...
              <div className="bg-white rounded-md shadow p-6 space-y-6">
                ...
                {[
                  ...
                  <div key={key}>
                    ...
                    <input type="text" value={(formData.social as any)[key]} onChange={(e) => handleSocialUpdate(key, e.target.value)} placeholder={placeholder}
                      className="w-full px-3 py-2 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
                  </div>
                ))}
              </div>
